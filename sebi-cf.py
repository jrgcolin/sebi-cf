...
"""
Snowwhite is a stand-alone SEBI-CF prototype for CEOP-AEGIS WP3

Use the demo test case:
- demo.xml file
- demo directory containing a test dataset

To run (snowwhite.py should be +x):
./snowwhite.py demo.xml
Have a look at demo_log.txt for logs
...
__date__ = "2011-01-20"
__version__ = "0.1.3 (dopey)"

print 'SnowWhite ' + __version__
import congrid
import getopt
import lib as l
import numpy as n
...
def process(fproject):
    ...
    try:
        ...
        if myproj.kbMode == "Massman":
            myproj.logs += '\n\nRunning kB-1 model:'
            kB_1,z0h,kblogs = l.kB(cd,ct,fc,k,hg,hr,hs,hv,lai,ndvi,p0,pr,tr,ur,z0m)
            myproj.logs += '\n'+kblogs
